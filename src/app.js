...
function showLoad(){lb.classList.add('show');}function hideLoad(){lb.classList.remove('show');}
...
async function handleFile(branch,file){
  ...
  showLoad();
  try{
    let rows; const n=file.name.toLowerCase();
    ...
    else throw new Error('صيغة غير مدعومة — xlsx/csv/pdf فقط');
    await _upTick(branch,60);
    if(!rows||!rows.length) throw new Error('لم يتم العثور على بيانات في الملف');
    await _upTick(branch,85);
    setBranchData(branch,rows,file.name,'');
...
function renderDocs(d){
  ...
  document.getElementById('doctors-content').innerHTML='<div class="card"><div class="card-head"><div class="card-title"><span class="dot"></span> الأطباء — '+BRANCH_LABELS[STATE.active]+'</div><input class="input" id="docSearch" placeholder="🔎 ابحث..." style="max-width:280px;"></div><div class="table-wrap" style="max-height:600px;overflow-y:auto;"><table><thead><tr><th style="width:60px;">#</th><th>الطبيب</th><th>القسم</th><th>الكتابات</th><th>الأدوية</th><th>المرضى</th><th style="width:180px;">النسبة</th></tr></thead><tbody id="docTbl"></tbody></table></div></div>';
  function fill(rows){document.getElementById('docTbl').innerHTML=rows.map((x,i)=>'<tr class="clickable" data-doc="'+escapeAttr(x.name)+'"><td>'+rankBadge(i)+'</td><td><strong>'+escapeHtml(x.name)+'</strong></td><td><span class="tag blue">'+escapeHtml(x.section||'—')+'</span></td><td class="num">'+fmt(x.total)+'</td><td class="num">'+fmt(x.uniqueDrugs)+'</td><td class="num">'+fmt(x.patients)+'</td><td>'+barRow(x.total,max)+'</td></tr>').join('');document.querySelectorAll('#docTbl tr[data-doc]').forEach(tr=>tr.onclick=()=>showDoctor(tr.dataset.doc));}
  fill(d.doctors);
  if(d.doctors.length>80){const tw=document.querySelector('#doctors-content .table-wrap');if(tw){tw.style.maxHeight='560px';tw.style.overflowY='auto';}}const _ds=document.getElementById('docSearch');_ds.addEventListener('input',debounce(e=>{const q=_ds.value.trim().toLowerCase();fill(d.doctors.filter(x=>!q||x.name.toLowerCase().includes(q)||(x.section||'').toLowerCase().includes(q)));},260));;
}
function renderDrugs(d){
  const max=d.drugs[0]?d.drugs[0].total:1;
  document.getElementById('drugs-content').innerHTML='<div class="card"><div class="card-head"><div class="card-title"><span class="dot"></span> الأدوية — '+BRANCH_LABELS[STATE.active]+'</div><input class="input" id="drugSearch" placeholder="🔎 ابحث..." style="max-width:280px;"></div><div class="table-wrap" style="max-height:600px;overflow-y:auto;"><table><thead><tr><th style="width:60px;">#</th><th>الدواء</th><th>الكتابات</th><th>الأطباء</th><th>المرضى</th><th style="width:180px;">النسبة</th></tr></thead><tbody id="drugTbl"></tbody></table></div></div>';
  function fill(rows){document.getElementById('drugTbl').innerHTML=rows.map((x,i)=>'<tr class="clickable" data-drug="'+escapeAttr(x.name)+'"><td>'+rankBadge(i)+'</td><td><strong>'+escapeHtml(x.name)+'</strong></td><td class="num">'+fmt(x.total)+'</td><td class="num">'+fmt(x.doctorCount)+'</td><td class="num">'+fmt(x.patients)+'</td><td>'+barRow(x.total,max)+'</td></tr>').join('');document.querySelectorAll('#drugTbl tr[data-drug]').forEach(tr=>tr.onclick=()=>showDrug(tr.dataset.drug));}
  fill(d.drugs);const _drs=document.getElementById('drugSearch');_drs.addEventListener('input',debounce(e=>{const q=_drs.value.trim().toLowerCase();fill(d.drugs.filter(x=>!q||x.name.toLowerCase().includes(q)));},260));;
}
  ...
  if(hasPeriods){
    ...
    if(detailRows.length){
      setTimeout(()=>{
        ...
        try{ if(window._plCompChartObj) window._plCompChartObj.destroy(); }catch(e){}
        window._plCompChartObj = new Chart(cv.getContext('2d'),{
          type:'bar',
          data:{
...
function loadEmbedded(){
  if(!window.__EMBEDDED_RX__) return;const d=window.__EMBEDDED_RX__;
  /* الملفات الأحدث تحمل الفترات الكاملة؛ القديمة تحمل الصفوف الأساسية فقط */
  if(d.periods && Object.keys(d.periods).length){
    BRANCHES.forEach(b=>{
    ...
    BRANCHES.forEach(b=>{if(d.branches[b]&&d.branches[b].rows&&d.branches[b].rows.length){setBranchData(b,d.branches[b].rows,d.branches[b].reportName||BRANCH_LABELS[b],d.branches[b].reportDate||'');if(!STATE.active) STATE.active=b;}});
  }
  if(d.dt && typeof d.dt==='object' && typeof DT!=='undefined') DT.store=d.dt;
...
function runGlobalSearch() {
  ...
  loaded.forEach(b => {
    const d = STATE.data[b];
    d.doctors.forEach(doc => {
      let r = docMap.get(doc.name) || {name:doc.name, section:doc.section, total:0};
      r.total += doc.total; docMap.set(doc.name, r);
    });
    d.drugs.forEach(drug => {
      let r = drugMap.get(drug.name) || {name:drug.name, total:0};
      r.total += drug.total; drugMap.set(drug.name, r);
    });
  });

  const matchDocs = [...docMap.values()].filter(x => x.name.toLowerCase().includes(q)).sort((a,b) => b.total-a.total).slice(0,6);
  const matchDrugs = [...drugMap.values()].filter(x => x.name.toLowerCase().includes(q)).sort((a,b) => b.total-a.total).slice(0,6);
...
══════════════════════════════════════════ */
const _IDB_NAME='pharmdash', _IDB_STORE='kv', _IDB_KEY='state';
...
function _idbPut(key,val){ return _idbOpen().then(db=>new Promise((res,rej)=>{ const tx=db.transaction(_IDB_STORE,'readwrite'); tx.objectStore(_IDB_STORE).put(val,key); tx.oncomplete=()=>{db.close();res();}; tx.onerror=()=>{db.close();rej(tx.error);}; })); }
function _idbGet(key){ return _idbOpen().then(db=>new Promise((res,rej)=>{ const tx=db.transaction(_IDB_STORE,'readonly'); const r=tx.objectStore(_IDB_STORE).get(key); r.onsuccess=()=>{db.close();res(r.result);}; r.onerror=()=>{db.close();rej(r.error);}; })); }
function _idbDelete(key){ return _idbOpen().then(db=>new Promise((res,rej)=>{ const tx=db.transaction(_IDB_STORE,'readwrite'); tx.objectStore(_IDB_STORE).delete(key); tx.oncomplete=()=>{db.close();res();}; tx.onerror=()=>{db.close();rej(tx.error);}; })); }
...
function saveData(){
  ...
  _saveTimer=setTimeout(()=>{
    try{ _idbPut(_IDB_KEY,_buildSnapshot()).catch(e=>console.error('saveData',e)); }
...
async function loadSavedData(){
  ...
  }catch(e){ console.error('loadSavedData',e); }
