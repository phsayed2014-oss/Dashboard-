...
function loadEmbedded(){
  ...
  try{
    ...
    BRANCHES.forEach(b=>{
      if(!periods[b].length)return;
      STATE.periods[b]=periods[b].map(p=>({id:p.id,label:p.label,rows:p.rows,parseMeta:p.parseMeta||null,_main:!!p._main,data:aggregate(p.rows||[])}));
