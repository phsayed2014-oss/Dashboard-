...
async function handleFile(branch,file){
  ...
  try{
    ...
    const prepared=await aggregateAsync(rows,p=>{const fill=document.getElementById('uprog-'+branch+'-fill'),pctEl=document.getElementById('uprog-'+branch+'-pct'),v=Math.round(60+p*25);if(fill)fill.style.width=v+'%';if(pctEl)pctEl.textContent=v+'%';});
    if(version!==_uploadVersion[branch])return;
...
async function aggregateAsync(rows,onProgress){
...
async function normalizeParsedRows(rawRows,source){
...
async function handlePeriodFile(branch, label, file) {
  ...
  try {
    ...
    validateParsedRows(rows);
    const id = Date.now();
    const agg=await aggregateAsync(rows,p=>{const fill=document.getElementById('uprog-'+branch+'-fill'),pctEl=document.getElementById('uprog-'+branch+'-pct'),v=Math.round(65+p*25);if(fill)fill.style.width=v+'%';if(pctEl)pctEl.textContent=v+'%';});
    if(version!==_uploadVersion[branch])return;
